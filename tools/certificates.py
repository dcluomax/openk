...
用法：
    python -m tools setup cert                       # 自动探测本机地址
    python -m tools setup cert 192.0.2.10 nas.example
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="生成自签 HTTPS 证书；未给地址时探测本机地址")
    parser.add_argument("names", nargs="*", help="证书覆盖的 IP 地址或主机名")
    ...
    try:
        ...
        print("缺少依赖：python -m pip install cryptography", file=sys.stderr)
        return 1
    names = args.names or [*local_ips(), "localhost", socket.gethostname()]
