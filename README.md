...
麦克风一旦外放，就有了「音箱 → 空气 → 麦克风 → 放大 → 音箱」这个环路。
环路增益超过 1，房间里某个共振频率就会指数级涨到削顶为止——就是那声尖叫。
限幅器只能让它没那么伤耳朵，**治不了成因**。

好在啸叫和唱歌在频谱上长得完全不一样，这让它可以被自动识别：

| | 啸叫 | 人在唱歌 |
|---|---|---|
| 谱形 | **一根孤零零的针**，能量全堆在一个频点 | **一把梳子**：基频 + 2f + 3f… 一串泛音 |
| 频率 | 锁死在房间共振上，纹丝不动 | 就算长音也有揉弦、有漂移 |
| 走势 | 只涨不落，直到削顶 | 有起有伏 |

三条同时满足才判定（缺一不可，否则会把长音误伤成啸叫），然后分两级处置：

1. **挖窄坑**（首选）：在啸叫的那个频率上挂一个 Q=24 的窄带陷波，把环路增益压回 1 以下。
   坑很窄，听感上几乎察觉不到，**音量不用降**。同一处反复叫就一档档挖深（-15 → -26 → -38 dB），
   最多同时挖 4 处；某处 12 秒不再复发就把坑填回去，免得唱着唱着声音被掏空。
2. **临时掐小外放**（兜底）：陷波都挖了还照叫，说明麦克风基本贴在音箱上了，
   或者共振点在乱窜。这时 40ms 内把外放掐到 22%，安静 1.2 秒后用 2.5 秒慢慢还回来
   （还得慢，快了一放开就又炸）。界面上会明确提示，让人知道该把麦拿远点。

陷波和急掐**只作用于监听支路，不碰录音**：环路一断麦克风自然就不再拾到啸叫，
...
│       ├── audio_quality.py  # 从频谱反推源音质（除重时判断留哪一版）
...
> 内容因账号而异，**摊不平**——但如果链接上带着 `v=`（从电台里点开某首歌复制出来的
> 就是这种），会退化成「只导入这一首」，不用回去手工删 `&list=` 参数。
> 私有歌单需要配 `OPENK_COOKIEFILE`。
>
...
- **跳过 `_` 和 `.` 开头的子目录**：留给存放不想被导入的文件（除重淘汰下来的版本就放在
  `_重复/`）。想看里面有什么，在目录选择器里明确点进去仍然可以。- **自动去重**：yt-dlp 下载的文件名里带 11 位视频 ID（`歌名 [dQw4w9WgXcQ].mp4`），
...
同一首歌从不同播放列表导进来好几遍是常事。`tools/dedupe.py` 把歌手＋歌名相同的
...
`instrumental.mp3` 和 `vocals.mp3` 字节数都完全一样，源是 128kbps 还是无损根本看不出来。
而且源文件在早期版本里分离完就被删了，很多歌压根没得比。

**改看频谱。** 有损编码会在某个频率上砖墙式切掉高频，这条线会原样穿过分离保留下来。
实测本曲库的截止基本都落在 16kHz 附近（YouTube 那档 AAC 的特征），所以光看截止线
区分度很小，真正拉开差距的是 **8–16kHz 还剩多少能量**——翻录、二次转码、过度压限
都会把它啃掉，实测跨度有 -10dB 到 -46dB。两项加上削波比例合成一个分数。

音质分差在 4 分以内（约等于截止差 1kHz 或高频差 4dB，听感上分不出）就算平手，
**改看歌词行数**——两版音质都一样时，当然留歌词更全的那个。

同名不同歌手当作翻唱各自保留——譚詠麟 和 周華健 的《朋友》是两首不同的歌。
只有两条都没填歌手时才退而只按歌名判，这种组在报告里会标上「⚠️ 歌手未知」，看过再决定。

被淘汰的版本**源文件不删，移进同目录的 `_重复/`**，挑错了还能捡回来；扫描会跳过
所有 `_` 或 `.` 开头的子目录，所以它们不会被当成新歌再导一遍（明确点进这个目录还是
看得到，就是留的后悔药）。频谱分析每首要解码一分钟音频，结果按文件的 mtime＋大小
缓存在 `data/quality-cache.json`，导入新歌后再跑一遍只算增量。
