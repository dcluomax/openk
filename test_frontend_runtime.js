...
async function fixture(t, options = {}) {
  ...
  w.eval(fs.readFileSync(path.join(__dirname, 'frontend/search.js'), 'utf8'));
  w.eval(script + `\nwindow.__t = {
...
test('metadata, no-lyrics badges, artist groups and language counts invalidate together', async (t) => {
  ...
  assert.equal(f.$('.song-row').children.length, 6, 'badge lives inside title cell, not an extra grid column');
