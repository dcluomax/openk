#!/usr/bin/env python3
"""Room contract tests: .venv/bin/python test_rooms.py (no ML models/server needed)."""
...
class RoomTests(unittest.TestCase):
    def setUp(self):
        self.root = Path(__file__).resolve().parent / (".room-test-" + uuid.uuid4().hex)
        self.root.mkdir()
...
def serve_frontend_fixture(port):
    ...
    root = Path(__file__).resolve().parent / (".room-js-test-" + uuid.uuid4().hex)
    root.mkdir()
    try:
