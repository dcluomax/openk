...
解析退化会直接表现为「按歌手」页面一片「未知歌手」。用例取自真实曲库里出现过的写法
...
真跑一遍，覆盖点歌台的关键链路：默认渲染文字列表、语种识别、点歌后常驻控制条出现、
默认伴唱、**返回曲库时歌不能停**、边唱边点、拼音首字母**前缀**匹配（搜 `dx` 只出「稻香」，
不出「淚的小雨」）、列表/大图切换、按歌手分组。这些都是只在交互之后才暴露的问题，
静态检查看不出来。
